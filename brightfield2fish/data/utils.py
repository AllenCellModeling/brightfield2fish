...
def float_to_eight_bit(im, dtype=np.uint8):
    """im is a np array"""
    imax = np.iinfo(dtype).max + 1  # eg imax = 256 for uint8
    im = im * imax
    im[im == imax] = imax - 1
...
def prep_fish(image, channel=1, T=0, clip_percentiles=[0, 99.99], median_subtract=True):
...
def plot_prepped(img3d, reduce_3D_to_2D=partial(np.percentile, q=100, axis=0)):
    ...
    img2d = float_to_eight_bit(img2d)
